...
import {useAuthChange, AuthChangeEvent, useAuthStatus, useAuthInfo, useUser} from './hooks.tsx'
import { Flows, AuthenticatorType } from '@/auth/allAuth.tsx'
...
export function pathForFlow(flow: any, typ?: any): string {
    let key = flow.id
...
export function pathForPendingFlow(auth: any): string | null {
    const flow = auth.data.flows.find((flow: any) => flow.is_pending)
    if (flow) {
...
function navigateToPendingFlow(auth: any): JSX.Element | null {
...
export function AuthenticatedRoute({ children }: RouteProps): JSX.Element {
    ...
    const { is_onboarded = false } = useUser() || {};


    if (status.isAuthenticated) {
        if (!is_onboarded) {
        return <Navigate to={`${URLs.PENDING}?${next}`} />;
    }
        return <>{children}</>
    } else {
...
export function AuthChangeRedirector({ children }: RouteProps): JSX.Element {
